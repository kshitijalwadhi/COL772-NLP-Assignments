...
def get_vector(word):
    if "~" in word:
        ...
        if check_numeric(temp):
            return embeddings[TILDA_NUM_KEY]
        else:
            return embeddings[TILDA_KEY]
...
def get_scores(preds, gold):
    ...
    micro_f1 = f1_score(
        np.array(flatten_preds)[idx], np.array(flatten_gold)[idx], average="micro"
    )
    macro_f1 = f1_score(
        np.array(flatten_preds)[idx], np.array(flatten_gold)[idx], average="macro"
    )
...
def train_model(model, epochs):
    ...
    for epoch in range(epochs):
        ...
        training_loss = train_one_epoch(
            model, trainDataLoader, optimizer, loss_function
        )
...
if __name__ == "__main__":
    ...
    trainDataLoader = DataLoader(
        trainData, batch_size=BATCH_SIZE, shuffle=False, collate_fn=custom_collate
    )
    valDataLoader = DataLoader(
        valData, batch_size=BATCH_SIZE, shuffle=False, collate_fn=custom_collate
    )
